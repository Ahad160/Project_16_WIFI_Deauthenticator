...
class Configuration(object):
    """ Stores configuration variables and functions for defite. """
    ...
    def parse_wpa_args(cls, args):
        ...
        if args.wordlist:
            if not os.path.exists(args.wordlist):
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} was not found, defite will NOT attempt to crack '
                ...
                cls.wordlist = None
                Color.pl('{+} {C}option:{O} wordlist {R}%s{O} is a directory, not a file. defite will NOT attempt to '
